...
async function deployContract(contractName, constructorArgs) {
  ...
  const factory = new ethers.Contract(process.env.CREATE2_MAINET_FACTORY, factoryAbi, deployer);

  // Get the Contract to be deployed via CREATE2
  const Contract = await ethers.getContractFactory(contractName);
  if (!Contract || !Contract.bytecode) {
    console.error(`${contractName} contract not found or not compiled correctly`);
    return;
  }
  
  // Get the init code for the contract
  const deployTransaction = await Contract.getDeployTransaction(...constructorArgs);
  ...
  console.log(`${contractName} contract deployed via CREATE2 at address:`, deployedContractAddress);
}
